...
>
> 本项目按**十个步骤**组织，完整走通一条**「先做出高指标 → 发现指标是假的 → 定位泄漏 → 修复 → 在干净框架下重做」**的实践主线。
...
- **关键成果**：定位并修复了导致 AUC 虚高（**0.988**）的三类数据泄漏，重建为时间切分的无泄漏评估（真实 AUC **约 0.62**）
...
| 集合 | 新用户占比 | 新商户占比 | 新券占比 |
|---|---|---|---|
| 验证段（6 月） | 36.8% | 3.4% | **75.2%** |
| 测试段（7 月） | 13.7% | 2.3% | **91.3%** |

> 测试段 **91.3% 的券是没见过的**——O2O 场景里券 ID 时效性极强，这个衰减是业务真实存在的，
> 不是修复引入的缺陷，只是修复让它**第一次显形**。这也解释了为什么真实 AUC 只能到 0.6 量级。
...
真实场景下（6% 正样本 + 91% 的新券冷启动），一个诚实模型的 Precision 只能在 0.15 左右。
...
按 01 → 10 的顺序执行 `notebooks/` 下的 notebook 即可。
...
| 目录 | 内容 |
|---|---|
| `data_out/` | 中间数据集、模型 `.pkl`、测试集预测 `submission_final.csv` |
...
完成了一个从探索到评估的完整 O2O 券核销预测项目，并通过对数据泄漏的系统诊断，
把虚高的 AUC（0.988）修正到可信区间（约 0.62）——模型的「真实水平」远低于初版报告，
但**评估过程的可信度**大幅提升，结论第一次变得可以指导投放决策。

### 展望

- **逐行滚动聚合**：当前聚合按「训练段整体」统计，训练段内部仍含自身行；
  可改为按「该行领券时刻之前」逐行滚动聚合，做到训练集内也严格无泄漏
- **冷启动兜底**：测试段 91.3% 的券是新的。可引入商户类目、地域、券面额等**静态属性**，
  让没见过的券也有画像可用——这是本项目提升 AUC 最有希望的方向
